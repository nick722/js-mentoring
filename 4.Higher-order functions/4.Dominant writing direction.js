...
function computeDominantWritingDirection(text) {
  ...
  let ltr = 0;
  let result;
  for (let scriptFromText of scriptsAndNumberOfElementsInThatScipts ) {
    result = SCRIPTS.filter(scriptFromScripts => {
      console.log("scriptFromScripts.name:", scriptFromScripts.name);
      console.log("scriptFromText.name:", scriptFromText.name);
      return scriptFromText.name === SCRIPTS.scriptFromText})
  }
  return result;
}
